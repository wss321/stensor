...
__global__ void col2img_kernel(const int n, const Dtype* data_col,
...
void gpu_colgrad2grad(const Dtype* data_col, const int channels,
  ...
  col2img_kernel<Dtype><<<GET_BLOCKS(num_kernels),
  CUDA_NUM_THREADS>>>(
...
template void gpu_colgrad2grad<float>(const float* data_col, const int channels,
                                const int height, const int width, const int kernel_h, const int kernel_w,
                                const int pad_h, const int pad_w, const int stride_h,
                                const int stride_w, const int dilation_h, const int dilation_w,
                                float* data_im);
template void gpu_colgrad2grad<double>(const double* data_col, const int channels,
                                 const int height, const int width, const int kernel_h, const int kernel_w,
                                 const int pad_h, const int pad_w, const int stride_h,
                                 const int stride_w, const int dilation_h, const int dilation_w,
                                 double* data_im);
...
__global__ void col2ndgrad_kernel(const int n, const Dtype* data_col,
...
void gpu_col2ndgrad(const Dtype* data_col, const int num_spatial_axes,
  ...
  DCHECK_LT(num_spatial_axes, CUDA_NUM_THREADS);
  switch (num_spatial_axes) {
    case 1:
      col2ndgrad_kernel<Dtype, 1>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 2:
      col2ndgrad_kernel<Dtype, 2>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 3:
      col2ndgrad_kernel<Dtype, 3>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 4:
      col2ndgrad_kernel<Dtype, 4>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 5:
      col2ndgrad_kernel<Dtype, 5>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 6:
      col2ndgrad_kernel<Dtype, 6>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 7:
      col2ndgrad_kernel<Dtype, 7>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 8:
      col2ndgrad_kernel<Dtype, 8>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 9:
      col2ndgrad_kernel<Dtype, 9>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    case 10:
      col2ndgrad_kernel<Dtype, 10>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(im_size), CUDA_NUM_THREADS>>>(
          im_size, data_col, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_im);
      break;
    default:
      LOG(FATAL) << "gpu_col2ndgrad does not support computation with "
...
// Explicit instantiation
template void gpu_col2ndgrad<float>(const float* data_col,
                                   const int num_spatial_axes, const int im_size,
                                   const int* im_shape, const int* col_shape,
                                   const int* kernel_shape, const int* pad, const int* stride,
                                   const int* dilation, float* data_im);
template void gpu_col2ndgrad<double>(const double* data_col,
                                    const int num_spatial_axes, const int im_size,
                                    const int* im_shape, const int* col_shape,
                                    const int* kernel_shape, const int* pad, const int* stride,
