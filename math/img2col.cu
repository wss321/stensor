...
__global__ void img2col_kernel(const int n, const Dtype *data_im,
...
void gpu_img2col(const Dtype *data_im, const int channels,
  ...
  img2col_kernel<Dtype><<<GET_BLOCKS(num_kernels),
  CUDA_NUM_THREADS>>>(
...
template void gpu_img2col<float>(const float *data_im, const int channels,
                                 const int height, const int width, const int kernel_h, const int kernel_w,
                                 const int pad_h, const int pad_w, const int stride_h, const int stride_w,
                                 const int dilation_h, const int dilation_w, float *data_col);
template void gpu_img2col<double>(const double *data_im, const int channels,
                                  const int height, const int width, const int kernel_h, const int kernel_w,
                                  const int pad_h, const int pad_w, const int stride_h, const int stride_w,
                                  const int dilation_h, const int dilation_w, double *data_col);

template<typename Dtype, int num_axes>
__global__ void ndimg2col_kernel(const int n, const Dtype *data_im,
...
void gpu_ndimg2col(const Dtype *data_im, const int num_spatial_axes,
  ...
  switch (num_spatial_axes) {
    case 1:
      ndimg2col_kernel<Dtype, 1>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 2:
      ndimg2col_kernel<Dtype, 2>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 3:
      ndimg2col_kernel<Dtype, 3>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 4:
      ndimg2col_kernel<Dtype, 4>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 5:
      ndimg2col_kernel<Dtype, 5>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 6:
      ndimg2col_kernel<Dtype, 6>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 7:
      ndimg2col_kernel<Dtype, 7>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 8:
      ndimg2col_kernel<Dtype, 8>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 9:
      ndimg2col_kernel<Dtype, 9>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    case 10:
      ndimg2col_kernel<Dtype, 10>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
          num_kernels, data_im, im_shape, col_shape,
          kernel_shape, pad, stride, dilation, data_col);
      break;
    default:LOG(FATAL) << "gpu_ndimg2col does not support computation with "
...
// Explicit instantiation
template void gpu_ndimg2col<float>(const float *data_im,
                                   const int num_spatial_axes, const int col_size,
                                   const int *im_shape, const int *col_shape,
                                   const int *kernel_shape, const int *pad, const int *stride,
                                   const int *dilation, float *data_col);
template void gpu_ndimg2col<double>(const double *data_im,
                                    const int num_spatial_axes, const int col_size,
                                    const int *im_shape, const int *col_shape,
                                    const int *kernel_shape, const int *pad, const int *stride,
