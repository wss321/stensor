...
void Pooling2d::forward_gpu() {
  ...
  switch (pool_type_) {
    ...
    case MEANPOOL:
      // NOLINT_NEXT_LINE(whitespace/operators)
      AvePoolForward<float><<<GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
          count, bottom_data, inputs_[0]->shape(0), channels_,
          height_, width_, pooled_height_, pooled_width_, kernel_h_,
          kernel_w_, stride_h_, stride_w_, padding_h_, padding_w_, top_data);
      break;
...
void Pooling2d::backward_gpu() {
  ...
  switch (pool_type_) {
    ...
    case MEANPOOL:
      // NOLINT_NEXT_LINE(whitespace/operators)
      AvePoolBackward<float><<<GET_BLOCKS(count), CUDA_NUM_THREADS>>>(
